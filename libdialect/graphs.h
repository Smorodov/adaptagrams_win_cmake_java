...
struct BoundingBoxDialect {
    ...
    BoundingBoxDialect(double xmin, double xmax, double ymin, double ymax)     
    {
        this->xmin = xmin;
        this->xmax=xmax;
        this->ymin = ymin;
    ...
    BoundingBoxDialect &operator+=(const BoundingBoxDialect &rhs);

    //! @brief  Write a simple representation of the bounding box.
    std::string repr(void) const;

    //! @brief  Get the width of the box.
    double w(void) const { return xmax - xmin; }

    //! @brief  Get the height of the box.
    double h(void) const { return ymax - ymin; }
    ...
    interval getInterval(vpsc::Dim dim) { return dim == vpsc::XDIM ? std::make_pair(xmin, xmax) : std::make_pair(ymin, ymax); }

    //! @brief  Get the centre of the box.
    Avoid::Point centre(void) const { return Avoid::Point((xmin+xmax)/2.0, (ymin+ymax)/2.0); }
...
public:
    ...
    BoundingBoxDialect getBoundingBox(
...
public:
    ...
    BoundingBoxDialect getBoundingBox(void) const;
...
// Global Operators
dialect::BoundingBoxDialect operator+(const dialect::BoundingBoxDialect &lhs, const dialect::BoundingBoxDialect &rhs);
bool operator==(const dialect::BoundingBoxDialect &lhs, const dialect::BoundingBoxDialect &rhs);
bool operator!=(const dialect::BoundingBoxDialect &lhs, const dialect::BoundingBoxDialect &rhs);
