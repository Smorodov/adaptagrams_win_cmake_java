...
BoundingBoxDialect operator+(const BoundingBoxDialect &lhs, const BoundingBoxDialect &rhs) {
    BoundingBoxDialect sum = lhs;
    sum += rhs;
    return sum;
}

bool operator==(const BoundingBoxDialect &lhs, const BoundingBoxDialect &rhs) {
...
BoundingBoxDialect &BoundingBoxDialect::operator+=(const BoundingBoxDialect &rhs) {
...
string BoundingBoxDialect::repr(void) const {
    return string_format("[%.2f, %.2f] x [%.2f, %.2f]", xmin, xmax, ymin, ymax);
}

LineSegment_SP BoundingBoxDialect::buildSideSegment(CardinalDir side) const {
    switch(side) {
    case CardinalDir::EAST:
        return std::make_shared<LineSegment>(Point(xmax, ymin), Point(xmax, ymax));
    case CardinalDir::WEST:
        return std::make_shared<LineSegment>(Point(xmin, ymin), Point(xmin, ymax));
    case CardinalDir::SOUTH:
        return std::make_shared<LineSegment>(Point(xmin, ymax), Point(xmax, ymax));
    case CardinalDir::NORTH:
        return std::make_shared<LineSegment>(Point(xmin, ymin), Point(xmax, ymin));
    }
}
...
BoundingBoxDialect Graph::getBoundingBox(const NodesById &ignore, bool includeBends) const {
...
string Graph::writeSvg(bool useExternalIds) const {
    ...
    BoundingBoxDialect b = getBoundingBox(ignore, includeBends);
    double pad = 8;
    b.xmin -= pad; b.xmax += pad; b.ymin -= pad; b.ymax += pad;
    ostringstream ss;
...
ColaGraphRep &Graph::updateColaGraphRep(void) {
    ...
    if (m_needNewRectangles) {
        ...
        size_t i = 0;
        for (auto p : m_nodes) {
            BoundingBoxDialect b = p.second->getBoundingBox();
            m_cgr.rs.push_back(new Rectangle(b.xmin, b.xmax, b.ymin, b.ymax));
...
void Graph::solidifyAlignedEdges(vpsc::Dim dim, const ColaOptions &opts) {
    ...
    while (it != m_edges.end()) {
        ...
        if (edgeIsAligned) {
            ...
            BoundingBoxDialect bsrc = src->getBoundingBox(),
