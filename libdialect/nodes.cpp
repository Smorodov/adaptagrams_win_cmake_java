...
BoundingBoxDialect Node::getBoundingBox(void) const {
    dimensions hd = getHalfDimensions();
    return BoundingBoxDialect(
...
string Node::writeSvg(bool useExternalId) const {
    BoundingBoxDialect b = getBoundingBox();
...
Polygon Node::makeLibavoidPolygon(void) const {
    Polygon poly(4);
    BoundingBoxDialect b = getBoundingBox();
    poly.setPoint(0, Point(b.xmin, b.ymin));
    poly.setPoint(1, Point(b.xmax, b.ymin));
    poly.setPoint(2, Point(b.xmax, b.ymax));
    poly.setPoint(3, Point(b.xmin, b.ymax));
    return poly;
}

bool Node::liesOppositeSegment(const LineSegment &seg, bool openInterval) {
    BoundingBoxDialect bb = getBoundingBox();
    double a = seg.varDim == vpsc::XDIM ? bb.xmin : bb.ymin,
           b = seg.varDim == vpsc::XDIM ? bb.xmax : bb.ymax;
