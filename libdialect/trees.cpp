...
Node_SP Tree::buildRootlessBox(CardinalDir growthDir) const {
    ...
    BoundingBoxDialect bb = m_graph->getBoundingBox();
    dimensions rootDims = m_root->getDimensions();
...
void Tree::addConstraints(Graph &G, bool alignRoot) {
    ...
    std::function<bool(Node_SP, Node_SP)> transCoordCmp;
    // Funciton to compare Nodes by their axial dimension:
    std::function<bool(Node_SP, Node_SP)> axialMeasureCmp;
    if (isVertical)
    {
        transCoordCmp = [](const Node_SP& a, const Node_SP& b)->bool { return a->getCentre().x < b->getCentre().x; };
        axialMeasureCmp = [](const Node_SP& a, const Node_SP& b)->bool { return a->getDimensions().second < b->getDimensions().second; };
    }
    else 
    {
        transCoordCmp = [](const Node_SP& a, const Node_SP& b)->bool { return a->getCentre().y < b->getCentre().y; };
        axialMeasureCmp = [](const Node_SP& a, const Node_SP& b)->bool { return a->getDimensions().first < b->getDimensions().first; };
    }
...
void Tree::addBufferNodesAndConstraints(Graph &G, NodesById &bufferNodes) {
    ...
    while (it != m_nodes.end() && jt != m_leafIDs.end()) {
        ...
        if (i > j) ++jt;
            ...
            if (i==j) {
                ...
                BoundingBoxDialect b = leaf->getBoundingBox();
                Node_SP bn = Node::allocate();
                switch(m_growthDir) {
                    case CardinalDir::EAST: bn->setBoundingBox(b.xmax, b.xmax + pad, b.ymin, b.ymax); break;
                    case CardinalDir::SOUTH: bn->setBoundingBox(b.xmin, b.xmax, b.ymax, b.ymax + pad); break;
                    case CardinalDir::WEST: bn->setBoundingBox(b.xmin - pad, b.xmin, b.ymin, b.ymax); break;
                    case CardinalDir::NORTH: bn->setBoundingBox(b.xmin, b.xmax, b.ymin - pad, b.ymin); break;
                }
    ...
    for (auto it = std::next(m_nodesByRank.begin()); it != m_nodesByRank.end(); ++it) {
        ...
        Node_SP first = rank.front(),
                last  = rank.back();
        BoundingBoxDialect a = first->getBoundingBox(),
                    b = last->getBoundingBox();
        // We make two buffer nodes, one for each end of the rank.
        Node_SP bn0 = Node::allocate(),
                bn1 = Node::allocate();
        if (isVertical) {
            bn0->setBoundingBox(a.xmin - pad, a.xmin, a.ymin, a.ymax);
            bn1->setBoundingBox(b.xmax, b.xmax + pad, b.ymin, b.ymax);
        } else {
            bn0->setBoundingBox(a.xmin, a.xmax, a.ymin - pad, a.ymin);
            bn1->setBoundingBox(b.xmin, b.xmax, b.ymax, b.ymax + pad);
        }
